...
const supaFetch = async (t, q = "") => { const r = await fetch(`${SUPA_URL}/${t}?${q}`, { headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` } }); if (!r.ok) throw new Error(`${r.status}`); return r.json(); };
const supaUpsert = async (t, d) => { const r = await fetch(`${SUPA_URL}/${t}`, { method: "POST", headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}`, "Content-Type": "application/json", Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(d) }); if (!r.ok) throw new Error(`${r.status}`); return r.json(); };
...
function Login({ onLogin }) {
  ...
  return (<div style={{ minHeight: "100vh", background: `radial-gradient(ellipse at 30% 20%, ${C.limeGlow}, transparent 60%), ${C.bg}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: bd }}>
    ...
    <div style={{ width: 380, ...crd, padding: "44px 36px", textAlign: "center" }}>
      ...
      <div style={{ textAlign: "left", marginBottom: 14 }}><label style={lbl}>Email</label><input type="email" value={e} onChange={x => setE(x.target.value)} style={inp} placeholder="you@example.com" /></div>
      <div style={{ textAlign: "left", marginBottom: 20 }}><label style={lbl}>Password</label><input type="password" value={p} onChange={x => setP(x.target.value)} style={inp} placeholder="••••••••" onKeyDown={x => x.key === "Enter" && (e && p ? onLogin() : setErr("Enter email and password"))} /></div>
      {err && <div style={{ color: C.red, fontSize: 12, marginBottom: 12 }}>{err}</div>}
      <button onClick={() => e && p ? onLogin() : setErr("Enter email and password")} style={{ width: "100%", padding: "12px 0", background: `linear-gradient(135deg, ${C.limeDark}, ${C.limeMuted})`, color: "#0c0d0f", border: "none", borderRadius: 8, fontSize: 14, fontFamily: hd, fontWeight: 700, cursor: "pointer", letterSpacing: "0.08em", textTransform: "uppercase" }}>Sign In</button>
    </div>
  </div>);
}
...
function IntakeForm({ project: p, onSave, onCancel }) {
  ...
  return (<div style={{ ...crd, padding: 24 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
      <div style={{ fontFamily: hd, fontWeight: 700, fontSize: 20, color: C.textPrimary, textTransform: "uppercase" }}>Install Intake</div>
      <div style={{ fontSize: 10, color: C.textMuted, fontFamily: mn, background: C.inputBg, padding: "4px 10px", borderRadius: 6 }}>On-site</div>
    </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 16, marginBottom: 20 }}>
      ...
      <div><label style={lbl}>Thermostat °F</label><input type="number" min="60" max="85" value={thermo} onChange={e => setThermo(Number(e.target.value))} style={inp} placeholder="72" /><div style={{ fontSize: 10, color: C.textMuted, marginTop: 4 }}>Set temp from thermostat</div></div>
    </div>
    ...
    <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
      ...
      <button onClick={() => onSave({ sq_ft: Number(sq), hvac_tons: Number(ht), seer: se, seer_source: ss, utility_rate: ur, insulation: JSON.stringify(insul), ceiling_u: computeU(insul), ac_type: acType, ac_count: acCount, thermostat_temp: thermo })} style={{ background: `linear-gradient(135deg, ${C.limeDark}, ${C.limeMuted})`, border: "none", borderRadius: 8, padding: "9px 24px", color: "#0c0d0f", fontSize: 13, fontFamily: hd, fontWeight: 700, cursor: "pointer", textTransform: "uppercase" }}>Save</button>
    </div>
  </div>);
}

export default function Dashboard() {
  const [auth, setAuth] = useState(false), [projects, setProjects] = useState([]), [readings, setReadings] = useState({});
  const [selId, setSelId] = useState(null), [form, setForm] = useState(false), [loading, setLoading] = useState(true);
  ...
  const loadP = useCallback(async () => { try { const d = await supaFetch("projects", "select=*&order=install_date.desc"); setProjects(d); if (d.length && !selId) setSelId(d[0].id); } catch (e) { console.error(e); } }, [selId]);
  const loadR = useCallback(async pid => { try { const d = await supaFetch("readings", `select=*&project_id=eq.${pid}&order=recorded_at.asc&limit=10000`); setReadings(p => ({ ...p, [pid]: d })); } catch (e) { console.error(e); } }, []);
  useEffect(() => { if (auth) { setLoading(true); loadP().then(() => setLoading(false)); } }, [auth, loadP]);
  useEffect(() => { if (selId && auth) loadR(selId); }, [selId, auth, loadR]);
  useEffect(() => { if (!auth || !selId) return; const i = setInterval(() => loadR(selId), 60000); return () => clearInterval(i); }, [auth, selId, loadR]);
  ...
  const save = async data => { try { await supaUpsert("projects", { id: selId, ...data }); await loadP(); setForm(false); } catch { alert("Save failed"); } };
  ...
  return (<div style={{ minHeight: "100vh", background: C.bg, color: C.textPrimary, fontFamily: bd }}>
    ...
    <div style={{ borderBottom: `1px solid ${C.border}`, padding: "12px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", background: C.surface, position: "sticky", top: 0, zIndex: 100 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div><div style={{ fontFamily: hd, fontWeight: 700, fontSize: 20, color: C.textPrimary, letterSpacing: "0.08em", textTransform: "uppercase" }}>Mallett Made</div><div style={{ fontSize: 9, color: C.lime, fontFamily: mn, letterSpacing: "0.2em" }}>RADIANT MONITOR</div></div>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
        <div style={{ textAlign: "right" }}><div style={{ fontSize: 9, color: C.textMuted, fontFamily: mn }}>ALL SITES · TOTAL</div><div style={{ display: "flex", gap: 16, alignItems: "baseline" }}><div style={{ fontSize: 22, fontWeight: 700, color: C.lime, fontFamily: hd }}>${totD.toFixed(2)}</div><div style={{ fontSize: 13, color: C.amber, fontFamily: mn }}>{totK.toFixed(0)} kWh</div></div></div>
        <div style={{ background: "rgba(127,255,0,0.06)", border: `1px solid ${C.limeBorder}`, borderRadius: 8, padding: "5px 14px", fontSize: 11, color: C.silver, fontFamily: mn }}>{projects.length} sites</div>
        <button onClick={() => setAuth(false)} style={btn(false)}>Sign Out</button>
      </div>
    </div>

    {loading ? <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 300 }}><style>{`@keyframes sp{to{transform:rotate(360deg)}}`}</style><div style={{ width: 32, height: 32, border: `3px solid ${C.border}`, borderTop: `3px solid ${C.lime}`, borderRadius: "50%", animation: "sp 0.8s linear infinite" }} /></div> : (
      <div style={{ display: "flex", height: "calc(100vh - 61px)" }}>
        <div style={{ width: 274, borderRight: `1px solid ${C.border}`, padding: "18px 14px", overflowY: "auto", flexShrink: 0, display: "flex", flexDirection: "column", gap: 8, background: C.surface }}>
          ...
          {projects.map(p => <SideCard key={p.id} project={p} sel={p.id === selId} onClick={() => { setSelId(p.id); setForm(false); loadR(p.id); }} readings={readings[p.id] || []} />)}
          {!projects.length && <div style={{ color: C.textMuted, fontSize: 13, padding: 12 }}>No projects yet.</div>}
        </div>

        {proj ? (<div style={{ flex: 1, overflowY: "auto", padding: "24px 28px", background: `radial-gradient(ellipse at 70% 10%, ${C.limeGlow}, transparent 50%), ${C.bg}` }}>
          <div style={{ marginBottom: 20, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <button onClick={() => setForm(!form)} style={btn(form)}>{form ? "✕" : "✎ Edit"}</button>
          ...
          {form && <div style={{ marginBottom: 24 }}><IntakeForm project={proj} onSave={save} onCancel={() => setForm(false)} /></div>}
        ...
        </div>) : (<div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ color: C.textMuted, fontSize: 16 }}>No project selected</div></div>)}
      </div>
    )}
  </div>);
}
